class Api::RepositoriesController < Api::ApiController
  ...
  def create
    ...
    if params[:name].nil?
      response = {:error => {:code => 400, :message => "Parameter 'name' is required!"}}
    ...
    if @repository.save
      ...
      response = {:error => {:code => 400, :message => "The repository named '#{params[:name]}' already exists for this api key!"}}
